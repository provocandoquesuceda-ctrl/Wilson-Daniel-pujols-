# Política de Privacidad

## Wilson Pujols - Portafolio Profesional

**Última actualización:** Septiembre 14, 2026

### 1. Introducción

Este documento describe cómo se recopila, utiliza y protege la información en nuestro sitio web. Tu privacidad es importante para nosotros.

### 2. Información que Recopilamos

#### 2.1 Información de Contacto
- Nombre
- Correo electrónico
- Número de teléfono
- *(Solo cuando voluntariamente contactas)*

#### 2.2 Datos de Navegación
- Información sobre cómo accedes al sitio
- Páginas visitadas
- Tiempo en el sitio
- Referente

#### 2.3 Almacenamiento Local
- Las tareas agregadas se almacenan SOLO en tu navegador
- Utilizamos LocalStorage (tecnología del navegador)
- No se envían a ningún servidor

### 3. Uso de la Información

Utilizamos la información para:
- Responder a tus consultas legales
- Mejorar la experiencia del usuario
- Entender cómo usas el sitio
- Enviar comunicaciones relevantes

### 4. Almacenamiento Local (LocalStorage)

**Características importantes:**

✅ Las tareas que creas se guardan SOLO en tu navegador
✅ No se envían a ningún servidor externo
✅ No compartimos tus datos con terceros
✅ Tienes control total sobre estos datos
✅ Puedes eliminarlas en cualquier momento

**Limitaciones:**

⚠️ Si limpias el cache/datos del navegador, se pierden las tareas
⚠️ No se sincronizan entre dispositivos
⚠️ No hay respaldo automático

### 5. Cookies

**Uso de Cookies:**
- Utilizamos cookies técnicas para mejorar la navegación
- Cookies de sesión (se eliminan al cerrar el navegador)
- Puedes desactivarlas en tu navegador

**Tipos de cookies:**
- Cookies de funcionalidad (necesarias para funcionar)
- Cookies analíticas (para entender el uso)

### 6. Servicios Exteriores

Nuestro sitio incluye enlaces a:
- WhatsApp
- Facebook
- LinkedIn
- Correo electrónico

Estos servicios tienen sus propias políticas de privacidad.

### 7. Google Analytics

(Si se implementa en el futuro)
- Datos anónimos de uso
- No se identifica información personal
- Puedes optar por no participar

### 8. Seguridad

**Medidas de Seguridad:**
- El sitio está alojado en GitHub Pages (HTTPS seguro)
- No almacenamos datos sensibles en servidores
- Tu información está protegida en tránsito (encriptada)
- No recopilamos datos de pago

### 9. Derechos del Usuario

Tienes derecho a:
- **Acceso:** Solicitar qué datos tenemos
- **Corrección:** Solicitar corrección de datos incorrectos
- **Eliminación:** Solicitar eliminación de datos
- **Transportabilidad:** Obtener copia de tus datos
- **Oposición:** Oponerte al procesamiento

### 10. Cambios en esta Política

Nos reservamos el derecho de actualizar esta política en cualquier momento.

- Los cambios significativos se notificarán
- Continuando el uso implica aceptación
- Última revisión: Septiembre 14, 2026

### 11. Contacto

Si tienes preguntas o inquietudes sobre privacidad:

**Email:** geronimo1636@omail.com
**WhatsApp:** 829-674-5162
**Ubicación:** SAN JOSÉ DE OCOA, República Dominicana

### 12. Cumplimiento Legal

Este sitio cumple con:
- Leyes dominicanas de protección de datos
- Estándares web internacionales
- Mejores prácticas de privacidad

---

**ACEPTACIÓN**

Al usar este sitio web, aceptas esta Política de Privacidad. Si no estás de acuerdo, por favor no uses el sitio.

**Última actualización: Septiembre 14, 2026**
